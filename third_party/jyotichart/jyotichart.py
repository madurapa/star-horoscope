...
import eastindian_transitchart as etc
import eastindian_numericalchart as enc
import support.general as gen
import support.languages as lang_module
import support.fonts as font_mod
import os
import tempfile
...
def get_planet_symbol(planet, language="english"):
    ...
    Parameters:
        ...
        language (str): Language code - "english" (default), "kannada", "hindi",
                        "sinhala" or "tamil".
...
def get_varga_name(varga, language="english"):
    '''Return a Shodashavarga division name in the given language.
    ...
    Returns:
        str: Division name in the requested language.
             e.g. get_varga_name("Navamsa", "sinhala") returns "නවාංශ".
             Unknown names are returned unchanged.
    '''
    return lang_module.get_varga_name(varga, language)


def _svg_string_via_draw(draw_fn):
    """Render a chart via its draw() into a temp dir and return the SVG XML.

    Lets every chart class offer to_svg_string() without duplicating the
    file-writing renderers. Returns an error string if not draw-ready.
    """
    with tempfile.TemporaryDirectory() as tmpdir:
        status = draw_fn(tmpdir + "/", "chart")
        if status != "Success":
            return status
        with open(os.path.join(tmpdir, "chart.svg"), encoding="utf-16") as f:
            return f.read()
...
class NorthChart:
    ...
    clr_houses = ['black', 'black', 'black', 'black', 'black', 'black', 'black', 'black', 'black', 'black', 'black', 'black'], font_family=None, font_sizes=None, clr_details='white'):
        self.chartcfg['aspect-visibility'] = aspect
        font_mod.apply_font_cfg(self.chartcfg, font_family, font_sizes)
        self.chartcfg['details-colour'] = clr_details
    ...
    def to_svg_string(self):
        '''Render the chart and return the SVG XML as a string, for embedding
        in HTML pages or reports, instead of writing a file. Returns an
        error string if the chart is not ready to be drawn.'''
        return _svg_string_via_draw(lambda location, filename: self.draw(location, filename, "svg"))
...
class SouthChart:
    def __init__(self, chartname, personname, IsFullChart = True, language = "english"):
        ...
        self.chartcfg["center-lagna-visibility"] = False
        self.ascendantsign = "NotSet"
        self.fullchart = IsFullChart
        self.dob = ""
        self.tob = ""
    ...
    clr_houses = ['black', 'black', 'black', 'black', 'black', 'black', 'black', 'black', 'black', 'black', 'black', 'black'], show_center_lagna=None, font_family=None, font_sizes=None, clr_details='white'):
        self.chartcfg['aspect-visibility'] = aspect
        if show_center_lagna is not None:
            # True -> localized Asc word; a string doubles as a custom line-2
            # label (e.g. "Navamsa"); False/None hides the centre label.
            self.chartcfg['center-lagna-visibility'] = show_center_lagna
        font_mod.apply_font_cfg(self.chartcfg, font_family, font_sizes)
        self.chartcfg['details-colour'] = clr_details
        self.chartcfg['background-colour'] = clr_background
        self.chartcfg['outerbox-colour'] = clr_outbox
...
################################### EAST CHART ###################################
class EastChart:
    '''East Indian (Maithili/Bengali) natal chart. Fixed signs, anti-clockwise.
    Aries is fixed at top-centre; other signs follow anti-clockwise to match
    east_indian.svg. Houses are counted anti-clockwise from the ascendant.'''
    def __init__(self, chartname, personname, IsFullChart=True, language="english"):
        self.chartname = chartname
        self.personname = personname
        self.language = language.lower() if language else "english"
        self.chartcfg = ec.reset_chartcfg()
        self.chartcfg["center-lagna-visibility"] = False
        self.ascendantsign = "NotSet"
        self.fullchart = IsFullChart
        self.dob = ""
    ...
    def __str__(self):
        return f"{self.chartname} east chart object of {self.personname}."
    ...
    clr_houses=['black', 'black', 'black', 'black', 'black', 'black', 'black', 'black', 'black', 'black', 'black', 'black'], show_center_lagna=None, font_family=None, font_sizes=None, clr_details='white'):
        ...
        self.chartcfg['house-colour']['libra'] = clr_houses[6]
        self.chartcfg['house-colour']['scorpio'] = clr_houses[7]
        self.chartcfg['house-colour']['saggitarius'] = clr_houses[8]
        self.chartcfg['house-colour']['capricorn'] = clr_houses[9]
        self.chartcfg['house-colour']['aquarius'] = clr_houses[10]
        self.chartcfg['house-colour']['pisces'] = clr_houses[11]
        return
    ...
    def add_planet(self, planet, symbol, housenum, retrograde=False, aspectsymbol="Default", colour='white'):
        ...
        self.planets[planet]["retro"] = retrograde
        sign = gen.get_signofsign(housenum, self.ascendantsign)
        pos = ec.get_coordniates(sign, self.planetindex[housenum-1])
        if pos == (0, 0):
            return(f'''Overflow Error: The given planet overflows the house. no position available in house diagram for planet {planet}.''')
        self.planetindex[housenum-1] = self.planetindex[housenum-1] + 1
        self.planets[planet]["pos"]["x"] = pos[0]
        ...
        for aspect in planet_aspects[planet]:
            aspecthousenum = gen.compute_nthsignnum(housenum, aspect)
            aspectsign = gen.get_signofsign(aspecthousenum, self.ascendantsign)
            asp_pos = ec.get_coordniates(aspectsign, self.planetindex[aspecthousenum-1])
            if asp_pos == (0, 0):
                return(f'''Overflow Error: The given planet aspect {planet} - {aspect} overflows the house. no position available in house diagram for planet.''')
            self.planetindex[aspecthousenum-1] = self.planetindex[aspecthousenum-1] + 1
            aspectpos = {"x": 0, "y": 0}
    ...
    def draw(self, location, filename, format='svg'):
        ...
        if self.__isObjectDrawReady() == False:
            return(f'''The chart is not ready to be drawn yet as all the needed inputs are not provided!!!''')
        return ec.create_chartSVG(self, location, filename, self.language)
...
################################### EAST TRANSIT CHART ###################################
class EastTransitChart:
    def __init__(self, chartname, personname, parentEastChart, IsFullChart=True, language="english"):
        self.chartname = chartname
        self.personname = personname
        self.language = language.lower() if language else "english"
        self.chartcfg = etc.reset_chartcfg()
        self.chartcfg["center-lagna-visibility"] = False
        self.ascendantsign = "NotSet"
        self.fullchart = IsFullChart
        self.parentEastChart = parentEastChart
        self.set_ascendantsign(self.parentEastChart.ascendantsign)
        self.transit_date = ""
        self.transit_time = ""
        self.outer_dob = ""
        self.outer_tob = ""
    ...
    def __str__(self):
        return f"{self.chartname} east transit chart object of {self.personname}."
    ...
    def add_planet(self, planet, symbol, housenum, retrograde=False, aspectsymbol="Default", colour='white'):
        ...
        self.planets[planet]["retro"] = retrograde
        sign = gen.get_signofsign(housenum, self.ascendantsign)
        pos = etc.get_transitcoordniates(sign, self.planetindex[housenum-1])
        if pos == (0, 0):
            return(f'''Overflow Error: The given planet overflows the house. no position available in house diagram for planet {planet}.''')
        self.planetindex[housenum-1] = self.planetindex[housenum-1] + 1
        self.planets[planet]["pos"]["x"] = pos[0]
        ...
        for aspect in planet_aspects[planet]:
            aspecthousenum = gen.compute_nthsignnum(housenum, aspect)
            aspectsign = gen.get_signofsign(aspecthousenum, self.ascendantsign)
            asp_pos = etc.get_transitcoordniates(aspectsign, self.planetindex[aspecthousenum-1])
            if asp_pos == (0, 0):
                return(f'''Overflow Error: The given planet aspect {planet} - {aspect} overflows the house. no position available in house diagram for planet.''')
            self.planetindex[aspecthousenum-1] = self.planetindex[aspecthousenum-1] + 1
            aspectpos = {"x": 0, "y": 0}
    ...
    def draw(self, location, filename, format='svg'):
        ...
        if self.__isObjectDrawReady() == False:
            return(f'''The chart is not ready to be drawn yet as all the needed inputs are not provided!!!''')
        return etc.create_transitchartSVG(self, location, filename, self.parentEastChart, self.language)
...
################################### EAST NUMERICAL CHART ###################################
class EastNumericalChart:
    '''East Indian numerical chart. One value per fixed sign (anti-clockwise).'''
    _sign_order = [
        "Aries", "Taurus", "Gemini", "Cancer",
        "Leo", "Virgo", "Libra", "Scorpio",
        "Saggitarius", "Capricorn", "Aquarius", "Pisces"
    ...
    def __init__(self, chartname, personname, language="english"):
        ...
        self.personname = personname
        self.language = language.lower() if language else "english"
        self.chartcfg = enc.reset_chartcfg()
        self.chartcfg["center-lagna-visibility"] = False
        self.ascendantsign = "NotSet"
        self.signvalues = {s.lower(): {"value": "", "colour": "lime"} for s in self._sign_order}
        return
    ...
    def __str__(self):
        return f"{self.chartname} east numerical chart object of {self.personname}."
    ...
    def draw(self, location, filename, format="svg"):
        ...
        if self.__isObjectDrawReady() == False:
            return(f'''The chart is not ready to be drawn yet as all the needed inputs are not provided!!!''')
        return enc.create_numericalchartSVG(self, location, filename, self.language)
